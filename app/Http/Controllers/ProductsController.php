...
class ProductsController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->type == 'is_variant') {
                $productRules['variants'] = [
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        ...
                        $all_paper_grams = array_column($variants, 'paperGram');
                        if ($all_paper_grams) {
                            foreach ($all_paper_grams as $paper_grams) {
                                if (empty($paper_grams)) {
                        ...
                        $all_paper_type = array_column($variants, 'paperType');
                        if ($all_paper_type) {
                            foreach ($all_paper_type as $paper_type) {
                                if (empty($paper_type)) {
                                    $fail('Variant type cannot be empty.');
                                    return;
                                }
                            }
                        } else {
                        ...
                        $all_paper_shade = array_column($variants, 'paperShade');
                        if ($all_paper_shade) {
                            foreach ($all_paper_shade as $paper_shade) {
                                if (empty($paper_shade)) {
                                    $fail('Variant shade cannot be empty.');
                                    return;
                                }
                            }
                        } else {
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->listing_type = $request['listingtype'];
                ...
                if ($request['type'] == 'is_variant') {
                    ...
                    if ($oldVariants->isNotEmpty()) {
                        ...
                        foreach ($request['variants'] as $variant) {
                            ...
                            $ProductVarDT->width = $variant['width'];
                            $ProductVarDT->weight = $variant['weight'];
                            $ProductVarDT->paper_grams = $variant['paperGram'];
                            $ProductVarDT->paper_type = $variant['paperType'];
                            $ProductVarDT->paper_shade = $variant['paperShade'];
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($Product->unit_id) {
            ...
            if (
                Unit::where('id', $Product->unit_sale_id)
                    ->where('deleted_at', '=', null)
                    ->first()
            ) {
            ...
            if (
                Unit::where('id', $Product->unit_purchase_id)
                    ->where('deleted_at', '=', null)
                    ->first()
            ) {
