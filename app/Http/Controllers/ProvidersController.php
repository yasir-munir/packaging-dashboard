...
class ProvidersController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($providers as $provider) {
            ...
            $item['person1'] = $provider->person1;
            $item['phone'] = $provider->phone;
            $item['tax_number'] = $provider->tax_number;
            $item['code'] = $provider->code;
            $item['email'] = $provider->email;
            $item['phone2_p1'] = $provider->phone2_p1;

            $item['country'] = $provider->country;
            $item['city'] = $provider->city;
            $item['adresse'] = $provider->adresse;

            $item['person2'] = $provider->person2;
            $item['phone1_p2'] = $provider->phone1_p2;
            $item['phone2_p2'] = $provider->phone2_p2;
            $item['email_p2'] = $provider->email_p2;

            $item['person3'] = $provider->person3;
            $item['phone1_p3'] = $provider->phone1_p3;
            $item['phone2_p3'] = $provider->phone2_p3;
            $item['email_p3'] = $provider->email_p3;

            $item['sales_tax_number'] = $provider->sales_tax_number;
            $item['state'] = $provider->state;
            $item['remarks'] = $provider->remarks;
            $item['status'] = $provider->status;
            $item['gl_acc_no'] = $provider->gl_acc_no;
            $item['tax_number'] = $provider->tax_number;
            $item['code'] = $provider->code;
    ...
    public function store(Request $request)
    {
        ...
        Provider::create([
            ...
            'phone2_p1' => $request['phone2_p1'],
            'person2' => $request['person2'],
            'email_p2' => $request['email_p2'],
            'phone1_p2' => $request['phone1_p2'],
            'phone2_p2' => $request['phone2_p2'],
            'person3' => $request['person3'],
            'email_p3' => $request['email_p3'],
            'phone1_p3' => $request['phone1_p3'],
            'phone2_p3' => $request['phone2_p3'],
            'sales_tax_number' => $request['sales_tax_number'],
            'state' => $request['state'],
            'remarks' => $request['remarks'],
            'status' => $request['status'],
            'gl_acc_no' => $request['gl_acc_no']
        ]);
    ...
    public function import_providers(Request $request)
    {
        ...
        $ext = pathinfo($file_upload->getproviderOriginalName(), PATHINFO_EXTENSION);
        if ($ext != 'csv') {
            return response()->json([
