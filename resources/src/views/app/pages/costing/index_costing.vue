<template>
  <div class="main-content">
    <breadcumb :page="$t('All Costings')" :folder="$t('Costing')"/>
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <router-link
              ...
              v-b-tooltip.hover
              title="View"
              :to="{ name:'detail_costing', params: { id: props.row.id} }"
            >
            ...
            <router-link
              ...
              v-b-tooltip.hover
              title="Edit"
              :to="{ name:'edit_costing', params: { id: props.row.id } }"
            >
          ...
          <b-row>
            ...
            <b-col md="12">
              <b-form-group :label="$t('PO Number')">
                <b-form-input
                  label="PO Number"
                  :placeholder="$t('Search by PO Number')"
                  v-model="Filter_code"
                ></b-form-input>
              </b-form-group>
            </b-col>
            ...
            <!-- Category  -->
            <b-col md="12">
              <b-form-group :label="$t('Export number')">
                <v-select
                  :reduce="label => label.value"
                  :placeholder="$t('Export Number')"
                  v-model="Filter_export_no"
  ...
  computed: {
    ...
    columns() {
      return [
        ...
        // {
        //   label: this.$t("Box Price"),
        //   field: "final_box_price",
        //   html: true,
        //   tdClass: "text-left",
        //   thClass: "text-left"
        // },
  ...
  methods: {
    ...
    Product_PDF() {
      ...
      let columns = [
        ...
        { title: "PO Number", dataKey: "po_number" },
      ...
      pdf.autoTable(columns, self.costings);
    ...
    Submit_import() {
      ...
      self.data.append("costings", self.import_products);
      axios
        .post("costing/import/csv", self.data)
        .then(response => {
    ...
    Reset_Filter() {
      ...
      this.Filter_code = "";
      this.Filter_po = "";
      this.Filter_export_no = "";
      this.Get_Products(this.serverParams.page);
    },

    // Simply replaces null values with strings=''
    setToStrings() {
      if (this.Filter_export_no === null) {
        this.Filter_export_no = "";
    ...
    Get_Products(page) {
      ...
      axios
        ...
        .then(response => {
          this.costings = response.data.costings;
    ...
    delete_by_selected() {
      this.$swal({
        ...
        if (result.value) {
          ...
          NProgress.set(0.1);
          axios
            .post("costing/delete/by_selection", {
